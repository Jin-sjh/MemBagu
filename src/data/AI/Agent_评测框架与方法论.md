...
## 【问题】
AI Agent 系统输出不确定、链路长，除了模型评估，软件层面的测试体系怎么搭？

## 【回答】
用**测试金字塔 + AI 特有评估**双层保障（以下为该文档工程实践口径，非行业标准）：
- **测试金字塔**：单测 60%（确定逻辑用 pytest + Mock LLM）、集成测 30%（检索管道/工具调用，testcontainers 起 Milvus/Redis）、E2E 10%。
- **AI 评估测试（特有）**：建评估集（500+ query-answer 对），指标含检索 Recall@10/Precision@5/NDCG@5、生成 BERTScore/ROUGE-L、端到端完成率/满意度；用 RAGAS 跑 RAG 管道。
- **进 CI**：每次提交自动跑评估防回归；每月一次全面压测。
- 要点：**确定性逻辑靠单测，模型质量靠评估集+自动化指标，二者都进 CI 才能既防 bug 又防回归**。
