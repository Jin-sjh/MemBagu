...
## 【回答】
最强模型往往**更贵、更慢**，且并非所有子任务都需要。路由能在**质量、成本、延迟、可用性、合规**之间做权衡；同时多供应商提高**容灾能力**，避免单点故障。

优先级调度策略可组合：固定优先级、成本优先（满足质量阈值下选最便宜）、延迟优先（SLA 内选最快）、负载感知（结合队列深度/429 率动态调整）、任务分类路由（代码类→A，摘要类→B）。优先级不是「永远用大模型」，而是在约束下最优化。

## 【问题】
你会如何设计一个多模型网关的架构？

## 【回答】
- **统一 Adapter 抽象**（每个厂商一个适配器，实现统一 `chat(messages, tools, model_id) -> Completion`）；
- **配置中心**维护 model_id → endpoint、密钥、RPM/TPM 限额、区域、价格表，含能力矩阵（JSON Mode/函数调用/视觉/长上下文）；
- 入口做**鉴权与租户路由**；
- 核心做**优先级调度 + 熔断 + 重试退避 + 降级链**；
- **全链路 Trace 与 usage 计费**；
- **密钥按租户与环境隔离**（国产与海外模型涉及数据出境与行业合规，路由也是合规路由）。
